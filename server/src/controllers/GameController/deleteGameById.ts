...
export const deleteGameById = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const { id } = req.params
    ...
    res.status(StatusCode.ok).json({ status: StatusRequestSuccess, data: deletedGame });
});