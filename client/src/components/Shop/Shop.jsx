...
export default class Shop extends Component {
  ...
  componentDidMount() {
    console.log(`${process.env.REACT_APP_API_URL}/games`);
    axios
  ...
  sortByName = () => {
    if (!this.state.ascendingSort || this.state.ascendingSort === "up") {
      this.setState({
        ...this.state,
        games: this.state.games.sort((a, b) => {
    ...
    if (this.props.userInSession.role === "ADMIN") {
      addGame = (
        <div className="flex addGame">
          <Link className="link" to="/addgame">
            <button>+</button>
          </Link>
        </div>
      );
    ...
    return (
      <div className="shop-container flex-column">
        ...
        <div className="gameList">
          <div class="form-section flex">{resultSearch}</div>
        </div>
      </div>
    );
  }
